...
### Requirement: Query workout routes with date range
The system SHALL expose a `GET /dashboard/workouts` endpoint that returns paginated GPX workout route records for the authenticated user within a date range.

#### Scenario: Workout records returned with pagination
- **WHEN** an authenticated user requests `GET /dashboard/workouts?start=2024-01-01&end=2024-01-31`
- **THEN** the system returns workout records (routeTime, waypointCount, start/end coordinates) sorted by routeTime, limited to 50 per page by default, with a `nextCursor` for pagination
...
#### Scenario: No workout records in range
- **WHEN** no GPX records exist within the requested date range
- **THEN** the system returns an empty `data` array with the period metadata

### Requirement: Direct query without scatter-gather
The system SHALL query workout records using a single DynamoDB Query on `PK=USER#{userId}` with `SK begins_with RECORD#GPX#`, since GPX records are not sharded.

#### Scenario: Single partition query
- **WHEN** a workouts query is executed
- **THEN** the system issues a single DynamoDB Query (not fan-out to multiple shards)

### Requirement: Validate workout query parameters
