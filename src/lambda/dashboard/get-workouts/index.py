"""GET /dashboard/workouts — Query GPX workout records with pagination."""
import os
import sys
import time
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """Lambda handler for GET /dashboard/workouts."""
    ...
    try:
        ...
        try:
            limit = validate_limit(params.get("limit"), WORKOUTS_LIMIT_DEFAULT, WORKOUTS_LIMIT_MAX)
        except ValueError as e:
            return create_error_response(
                error_message=str(e),
                status_code=400,
        ...
        # Query GPX workout records (non-sharded)
        result = query_non_sharded(
            user_id=user_id,
            sk_prefix="RECORD#GPX#",
            start_date=start,
            end_date=end,
            limit=limit,
            cursor=cursor,
        ...
        logger.info(
            "workouts_query: range=%s..%s, records=%d, elapsed_ms=%s",
            start, end, len(items), elapsed,
        )

        return create_success_response(
            data=items,
            message="Workout records retrieved successfully",
            metadata={
        ...
        logger.error("Error in get-workouts: %s", str(e))
        return create_error_response(
            error_message="Failed to retrieve workout records",
            status_code=500,
