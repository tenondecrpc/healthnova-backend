"""Unit tests for GET /dashboard/workouts Lambda."""
import json
import sys
import unittest
...
def _get_handler():
    mod_name = "get-workouts.index"
    if mod_name in sys.modules:
        del sys.modules[mod_name]
    return import_module(mod_name).handler


class TestGetWorkouts(unittest.TestCase):

    @patch("shared.query_utils.query_non_sharded")
    def test_pagination(self, mock_query):
        mock_query.return_value = {
            "items": [{"PK": "USER#abc", "SK": "RECORD#GPX#2024-01-01", "waypointCount": 150}],
            "last_key": {"PK": "USER#abc", "SK": "RECORD#GPX#2024-01-01"},
        }
